...
/*
 ...
 Modified by Silas Hatfield
 (website: silashatfield.com       email : silas@doxpress.com)
...
define("WEB_INVOICE_VERSION_NUM", "2.2.0");
...
global $_web_invoice_getinfo, $_web_invoice_payment_cache, $_web_invoice_payment_meta_cache, $_web_invoice_clear_cache;
...
{	
	...
	{
	    //More to come later
	    if(($_REQUEST['eqdkp_data'])) {
		setcookie('eqdkp_data');
	    };
	}
	...
	function invoice_overview()
	{
	    ...
	    {
		echo "<div id=\"message\" class='error' ><p>".__('cURL is not turned on on your server, credit card processing will not work. If you have access to your php.ini file, activate <b>extension=php_curl.dll</b>.', WEB_INVOICE_TRANS_DOMAIN)."</p></div>";
	    }
	    echo $Web_Invoice_Decider->display();
	}
	...
	function init()
	{
		...
		{	
		    ...
		    {
			if (web_invoice_does_invoice_exist($invoice_id) &&
			    get_option('web_invoice_force_https') == 'true' &&
			    $_SERVER['HTTPS'] != "on" &&
...
%content
	...
	function recipient($what)
	{
		...
		case 'country':
		    ...
		    break;
		case 'company_name':
		    if (get_usermeta($uid,'company_name'))
		    {
		    ...
		    break;
		case 'tax_id':
		    if (get_usermeta($uid,'tax_id'))
		    {
			return get_usermeta($uid,'tax_id');
		    } else {
			return "";
		    }
		    break;

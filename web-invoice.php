...
class Web_Invoice {
	   ...
	   function SetUserAccess($level = 8) {
		...
		function admin_head() {
		echo "<link rel='stylesheet' href='".$this->uri."/css/wp_admin-1.0.0.css' type='text/css'type='text/css' media='all' />";
		}
		...
		function web_invoice_add_pages() {
			...
			add_submenu_page( __FILE__, "Manage Invoice", "New Invoice", $this->web_invoice_user_level, 'new_invoice', array(&$this,'new_invoice'));
			add_submenu_page( __FILE__, "Recurring Billing", "Recurring Billing", $this->web_invoice_user_level, 'recurring_billing', array(&$this,'recurring'));
			add_submenu_page( __FILE__, "Settings", "Settings", $this->web_invoice_user_level, 'invoice_settings', array(&$this,'settings_page'));
		}
		...
		function recurring() {
			$Web_Invoice_Decider = new Web_Invoice_Decider('recurring_billing');
			if($this->message) echo "<div id=\"message\" class='error' ><p>".$this->message."</p></div>";
			echo $Web_Invoice_Decider->display();
		}
		...
		function settings_page() {
			$Web_Invoice_Decider = new Web_Invoice_Decider('invoice_settings');
			if($this->message) echo "<div id=\"message\" class='error' ><p>".$this->message."</p></div>";
			echo $Web_Invoice_Decider->display();
		}
		...
		function install() {
			...
			foreach ($all_users_with_meta as $user)
				...
				if(get_usermeta($user, 'country')) { update_usermeta($user, 'country',get_usermeta($user, 'country')); delete_usermeta($user, 'country',''); }
				}
			}
...
class Web_Invoice_GetInfo {
	...
	global $wpdb;
	$uid = $wpdb->get_var("SELECT user_id FROM ".Web_Invoice::tablename('main')." WHERE invoice_num = ".$this->id);
	...
	global $wpdb;
	$invoice_info = $wpdb->get_row("SELECT * FROM ".Web_Invoice::tablename('main')." WHERE invoice_num = ".$this->id);

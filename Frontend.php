...
function web_invoice_the_content($content) {
	...
	if(!web_invoice_md5_to_invoice($_GET['invoice_id'])) return $content; else $invoice_id = web_invoice_md5_to_invoice($_GET['invoice_id']);
	...
	if(!web_invoice_recurring($invoice_id))  web_invoice_show_invoice_overview($invoice_id);

	// Show this if recurring
	if(web_invoice_recurring($invoice_id))  web_invoice_show_recurring_info($invoice_id);
...
jQuery('#web_invoice_process_wait span').html('<img src="<?php echo Web_Invoice::frontend_path(); ?>/core/images/processing-ajax.gif">');
...
<?php if(get_option('web_invoice_use_css') == 'yes') { ?>
...
#invoice_page #web_invoice_itemized_table .alt_row {background: #EFEFEF}
...
#invoice_page #web_invoice_itemized_table th {background: #DFDFDF}
...
#invoice_page .web_invoice_due_date {top:0;right:0;margin:0;padding:0;}
